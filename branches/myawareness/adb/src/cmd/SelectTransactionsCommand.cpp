    ...
    void SelectTransactionsCommand::buildSqlCommand()
    {
        ...
        if (parameters_) {
            ...
            if (parameters_->hasNamePattern()) {
                sout << "JOIN [" << Configuration::TABLE_DESCRIPTIONS << "] ";
                sout << "ON [" << Configuration::COLUMN_DESCRIPTION << "] = [" << Configuration::TABLE_DESCRIPTIONS << "].[" << Configuration::COLUMN_ID << "] ";
            }
        }
        ...
        if (parameters_) {
            if (parameters_->getItemId() != Configuration::DEFAULT_ID) {
                sout << "AND [" << Configuration::COLUMN_DESCRIPTION << "] = " << parameters_->getItemId() << " ";
            } else {
                ...
                if (parameters_->hasNamePattern()) {
                    sout << "AND [" << Configuration::TABLE_DESCRIPTIONS << "].[" << Configuration::COLUMN_NAME << "] LIKE '%" << parameters_->getNamePattern() << "%' ";
                }
            }
        }
