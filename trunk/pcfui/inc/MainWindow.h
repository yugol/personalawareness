...
public:
    ...
    void setDatabaseOpenedView(bool isOpened);
    void setUndoRedoView(const ReversibleDatabaseCommand* undo,
            const ReversibleDatabaseCommand* redo);
    void scrollTransactionListAtEnd();

    void refreshStatement(const std::vector<std::pair<const Account*, double> >& statement,
            double netWorth);
    void populateCashAccounts(const std::vector<const Account*>& accounts);
    void appendIncomeAccounts(const std::vector<const Account*>& accounts);
    void appendExpensesAcounts(const std::vector<const Account*>& accounts);
    void populateItems(const std::vector<const Item*>& items);
    void populateTransactions(const wxArrayString& entries);
...
protected:
    ...
    void onOptimizeDatabase(wxCommandEvent& event);
