...
void DatabaseConnection::readPreferences(SelectPreferences& prefs)
{
	...
	Configuration::instance()->setHideZeroBalanceAccounts(prefs[Configuration::PREF_HIDE_ZERO_BALANCE_ACCOUNTS].c_str());
}
...
void DatabaseConnection::writePreferences(sqlite3* database)
{
	...
	UpdatePreference(database, Configuration::PREF_HIDE_ZERO_BALANCE_ACCOUNTS, ((b) ? ("1") : ("0"))).execute();
}
