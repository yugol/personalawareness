...
void SelectTransactions::buildSqlCommand()
{
	...
	if (parameters_) {
		if (parameters_->isLastTransactionOnly()) {
			sout << "ORDER BY [" << Configuration::COLUMN_DATE << "] DESC ";
			sout << "LIMIT 1;" << endl;
		} else if (parameters_->isFirstTransactionOnly()) {
			sout << "ORDER BY [" << Configuration::COLUMN_DATE << "] ASC ";
			sout << "LIMIT 1;" << endl;
		} else if (parameters_->isCheckUsage()) {
			sout << "LIMIT 1;" << endl;
		} else {
